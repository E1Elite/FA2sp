...
void CurrentScript::LoadExtraParamBox(ppmfc::CComboBox& comboBox, int actionIndex)
{
	...
	if (hasExtraParam)
	{
		if (param < 0 && CScriptTypeParamCustom::ExtParamsCustom[-param].HasExtraParam_ > 1)
		{
			...
			if (auto entities = fadata.GetSection(CScriptTypeParamCustom::ExtParamsCustom[-param].XtraSection_))
			{
				int xtraIndex = 0;
				for (auto& xtra : entities->GetEntities())
				{
					comboBox.SetItemData(comboBox.AddString(xtra.second), xtraIndex++);
				}
		...
		{
			comboBox.SetItemData(comboBox.AddString(Translations::TranslateOrDefault("ScriptExtraParam.Preference.0", "0 - Least Threat")), 0);
			comboBox.SetItemData(comboBox.AddString(Translations::TranslateOrDefault("ScriptExtraParam.Preference.1", "1 - Highest Threat")), 1);
